...
import { AskInbox, card, plain } from './ui.ts';
...
export function registerNativeAsks(pi: ExtensionAPI) {
  ...
  let open: { epoch: number; close?: () => void } | undefined;
  ...
  function invalidate() {
    ...
    const old = open; open = undefined; old?.close?.();
  }
  ...
  function resumeAfterBoundary(ctx: ExtensionContext, mine: number) {
    if (timer) return;
    timer = setTimeout(() => {
      timer = undefined;
      if (mine !== epoch || !sameSession(ctx)) return;
      if (!ctx.isIdle()) { resumeAfterBoundary(ctx, mine); return; }
      paused = false; navigating = false; ambient(ctx); flush(ctx);
    }, 100);
    timer.unref?.();
  }
  ...
  for (const event of ['session_compact', 'session_compact_failed'] as const) {
    pi.on(event, (_event, ctx) => {
      clearTimeout(timer); timer = undefined; paused = false; ambient(ctx); flush(ctx);
    });
  }
  ...
  pi.registerTool({
    ...
    description: 'Leave one private native Pi question for the person, then continue working. Returns a stable pending identity immediately; it does not open UI or wait. The person voluntarily opens /asks and may respond freely, with optional suggested answers. Saved feedback steers this session or wakes it when idle. Interactive TUI only; questions remain local to this session/branch, not shared Threadroom.',
    ...
    renderResult(value) { return card(value.details?.status === 'pending'
      ? `Pending private ask · ${value.details.id} · /asks` : `Private ask: ${value.details?.status || 'unavailable'}`); },
  ...
  pi.registerCommand('asks', {
    description: 'Open this session’s private pending questions. Esc closes without answering; unsaved drafts are discarded.',
    async handler(args, ctx) {
      ...
      if (open) { ctx.ui.notify('A private question inbox is already open.', 'info'); return; }
      const state = project(ctx);
      ...
      const mine = epoch;
      const opening = { epoch: mine, close: undefined as (() => void) | undefined };
      open = opening;
      try {
        const feedback = await ctx.ui.custom<{ id: string; text: string; optionIndex?: number } | undefined>((tui, theme, _keys, done) => {
          opening.close = () => done(undefined);
          if (!active(ctx, mine)) { done(undefined); }
          return new AskInbox(state.pending, tui, theme, done, initialId);
        }, { overlay: true, overlayOptions: { width: '90%', maxHeight: '90%', anchor: 'center' } });
        if (!active(ctx, mine)) return;
        if (!feedback) { ctx.ui.notify('Closed without answering. Question stays pending; unsaved draft discarded.', 'info'); return; }
        const current = project(ctx);
        const question = current.questions.get(feedback.id);
        if (!question || current.answers.has(feedback.id) || !feedback.text.trim()) return;
        ...
        pi.appendEntry(ANSWER, answer); // Save prompt association + real structured answer BEFORE wake.
        if (project(ctx).answers.get(question.id)?.answerId !== answer.answerId) throw new Error('Host did not save the answer entry.');
        ambient(ctx); flush(ctx);
      } catch (error) {
        if (active(ctx, mine)) ctx.ui.notify(`Private question could not complete: ${plain(error)}. Check /asks; no decline was recorded.`, 'error');
      } finally { if (open === opening) open = undefined; }
    },
  });
}
