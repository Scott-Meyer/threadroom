...
`/threadroom` shows the selected API and website addresses, connection state and session watches. Failed tool operations show those addresses too. With no explicit API configuration, the first shared operation or restored watch ensures a compatible detached local API + website is running. Private-only use remains network- and service-free. Explicit endpoints retain external ownership and are never replaced or auto-started.
