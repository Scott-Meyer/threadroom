...
expect(!manifest.bundleDependencies?.length, 'The optional app must not be installed as a bundled dependency.');
expect(!manifest.dependencies?.['threadroom-service'] && !manifest.optionalDependencies?.['threadroom-service'], 'Threadroom service must not be an eager npm dependency, including an optional dependency.');
expect(manifest.files?.includes('runtime/'), 'The portable service archive must be packaged for explicit shared-feature installation.');
